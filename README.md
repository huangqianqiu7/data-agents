...
import（旧 sibling 包不随 GitHub 提交；仅在本地归档存在时供 parity / 历史回归参考）。

> 本文档定位是 **仓库入口 + LangGraph 后端速查 + 开发者上手手册**，不重复本地 Phase 历史叙事。
...
根目录只保留运行、打包和入口文档需要的文件；本地计划材料、旧 sibling
backend 与运行产物不随 GitHub 提交。
...
**YAML 显式非空值则胜出**（用于本地 mock gateway 调试时覆盖）。
...
`tests/conftest.py` 在本地归档存在时会把 `src/废弃/` 加到 `sys.path`，让
legacy parity 测试可继续运行；干净 clone 中没有该目录时会忽略这些 legacy
测试。这与"运行时不依赖 sibling 包"并不冲突——本包源码不 import 它们即可。
