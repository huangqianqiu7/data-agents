...
> 本文档定位是 **仓库入口 + LangGraph 后端速查 + 开发者上手手册**，不重复 Phase 历史叙事。
> 阶段进度、设计动机、跨包决策以
> [`src/计划/LangChain/MIGRATION_STATUS.md`](src/计划/LangChain/MIGRATION_STATUS.md)
> 与 [`src/计划/LangChain/LANGCHAIN_MIGRATION_PLAN.md`](src/计划/LangChain/LANGCHAIN_MIGRATION_PLAN.md)
...
| 路径 | 用途 |
| --- | --- |
| `src/data_agent_langchain/` | 当前默认 LangGraph backend 源码 |
| `tests/` | 回归测试、packaging guard、RAG / runner / tool 行为测试 |
| `docker/` | 评测镜像辅助文件与 RAG 镜像变体 |
| `configs/` | 本地开发配置；受 `.gitignore` 保护，不提交 secret |
| `data/` | 本地 DABench 数据；受 `.gitignore` 保护 |
| `artifacts/` | gateway smoke、runner trace、metrics、prediction 等运行产物 |
| `src/计划/` | 设计、实施计划、审查记录与阶段追踪文档 |
| `src/废弃/` | legacy sibling backend 归档，只供 parity / 历史参考 |
| `old/` | 更早期 agent 原型归档 |

根目录只保留运行、打包和入口文档需要的文件；本地旧材料已归档到
`docs/local-archive/`（该目录被 `.gitignore` 忽略）。
...
| `config.py` | 1 | 7 个 frozen dataclass + `AppConfig` + YAML 加载 + 评测约束校验 |
| `constants.py` | 1 | 跨模块字符串常量（如 `FALLBACK_STEP_PROMPT`） |
| `exceptions.py` | 1 | 异常层次（根类 `DataAgentError`） |
| `submission.py` | 1 | 容器提交入口：从 `/input` / `/output` / `/logs` 与 `MODEL_*` 环境变量构造生产态 `AppConfig` |
| `agents/` | 22 个 `.py` | LangGraph 节点实现：gate / model / parse_action / tool / advance / finalize / planner / replanner / corpus_recall / task_entry，加 ReAct / Plan-and-Solve 顶层图与 execution_subgraph |
...
| `memory/` | 27 个 `.py` | working memory、v2 dataset memory、JSONL store、精确召回器，以及 M4 `memory/rag/` per-task corpus RAG |
...
```text
START → task_entry → execution → finalize → END
```

`task_entry` 在进入 ReAct T-A-O 循环前做一次入口召回：先取 v2 dataset facts，
再按当前 task 的 context 文档取 corpus snippets。`execution` 是已编译的
`execution_subgraph`；ReAct 没有 plan / replanner，子图返回 `replan_required`
时直接走 finalize。
...
M4 Corpus RAG 只在 task 入口召回一次，避免 prompt 随 ReAct 步数膨胀：

```text
runner 子进程入口
  → build_embedder
  → build_task_corpus(task.context_dir)
  → set_current_corpus_handles

plan_solve: planner_node → recall_dataset_facts → recall_corpus_snippets
react:      task_entry   → recall_dataset_facts → recall_corpus_snippets
model_node: 只渲染 state["memory_hits"]，不主动召回
```
...
按项目安全约定，**LLM 服务地址与 API Key 不应硬编码进 YAML / 镜像**。
开发态 `load_app_config` 已支持 env-var 兜底，与提交态
...
本地开发态仍通过 `--config configs/local.yaml` 传入 YAML，但该文件只应保留
`config.py` 默认值无法知道的本地路径参数。当前推荐最小内容：
...
其它参数（例如 `agent.action_mode` / `agent.max_steps` / `run.max_workers` /
`run.output_dir` / `observability.gateway_caps_path`）默认都来自 `config.py`。
除非要临时做本地实验，否则不要在 `configs/local.yaml` 里重复覆盖这些默认值。

### 4. 准备 `gateway_caps.yaml`

第一次跑前先做一次能力探针（默认写 `artifacts/gateway_caps.yaml`）：

```bash
dabench-lc gateway-smoke --config configs/local.yaml
```

如需临时写到其它位置，可显式传 `--output <path>`；常规本地运行建议仍使用
`observability.gateway_caps_path` 对应的默认位置。

> `configs/local.yaml` 已被 `.gitignore` 保护；YAML 中不要写 secret，让
> `MODEL_*` env vars 兜底即可。

### 5. 跑单个任务

```bash
dabench-lc run-task task_1 --config configs/local.yaml --graph-mode plan_solve
```

或：

```bash
python -m data_agent_langchain.cli run-task task_1 --config configs/local.yaml
```

开启 M4 per-task corpus RAG 时，需要安装可选依赖并显式打开 memory mode 与
RAG 开关：
...
`--memory-rag` 只在 `memory.mode=read_only_dataset` 或 `full` 时生效；
`memory.mode=disabled` 会强制走无 RAG baseline。容器提交路径默认
`memory.rag.enabled=false`，可通过 `DATA_AGENT_RAG=1` 显式打开。


本地 CLI 在最终配置启用 corpus RAG 且 `memory.mode` 为 `read_only_dataset`
或 `full` 时，会在启动 worker 前默认设置 `HF_HUB_OFFLINE=1` 与
`TRANSFORMERS_OFFLINE=1`，让 `sentence-transformers` 直接从本机
HuggingFace 缓存加载 `microsoft/harrier-oss-v1-270m`，避免并发 benchmark
worker 反复请求 `huggingface.co`。如果本机缓存缺失，可先临时设置
`HF_ENDPOINT=https://hf-mirror.com`，或取消离线变量后跑一次单任务预热缓存。

RAG 提交镜像使用 `docker/Dockerfile.rag`：build 阶段会联网下载
`microsoft/harrier-oss-v1-270m` 到 `/opt/models/hf` 并打包进镜像；runtime
...
```bash
dabench-lc run-benchmark --config configs/local.yaml --limit 10
```

`run-benchmark` 默认显示 baseline 风格的 `rich` 进度条；在日志采集、窄终端或
CI 场景下可用 `--no-progress` 关闭：
...
`--graph-mode` 可选 `plan_solve` 或 `react`；`--memory-rag` 需配合
`--memory-mode read_only_dataset` 或 `full` 才生效。
...
入口共存。开发态 `main()` 会尝试从 `.env` 加载 `MODEL_*` 等环境变量；容器
提交态走 `submission.py:main()`，不依赖 `.env`。

---

## 配置（`config.py`）

`AppConfig` 由 7 个 frozen dataclass 组成，全部 `frozen=True, slots=True`，
可 pickle、可在子进程间安全传递：
...
`CorpusRagConfig` 挂在 `MemoryConfig.rag` 下，默认 `enabled=false`，但保留
...
本地 CLI 与容器提交最终都会构造同一个 `AppConfig`，但入口不同：

| 场景 | 入口 | 参数来源 |
| --- | --- | --- |
| 本地 CLI | `dabench-lc ... --config configs/local.yaml` | `configs/local.yaml` 只覆盖本地路径；LLM 身份字段走 `MODEL_*` env 兜底；其余字段走 `config.py` dataclass 默认 |
| 容器提交 | `python -m data_agent_langchain.submission` | 不读 YAML；`MODEL_API_URL` / `MODEL_NAME` 必填，`MODEL_API_KEY` 可选；路径固定为 `/input` / `/output` / `/logs` / `/tmp/dabench-runs` / `/app/gateway_caps.yaml`；其余字段走 `config.py` dataclass 默认 |

本地 `load_app_config(configs/local.yaml)` 的实际优先级是：

```text
YAML 显式非空值 > MODEL_* 环境变量 > config.py dataclass 默认值
```

但 `MODEL_*` 只覆盖三个 LLM 身份字段：
...
当前容器生产入口不读取 `DABENCH_ACTION_MODE`、`DABENCH_MAX_WORKERS` 或其它
`DABENCH_*` 环境变量；`action_mode` 来自 `AgentConfig.action_mode` 默认
`tool_calling`，`max_workers` 来自 `RunConfig.max_workers` 默认 `5`。
...
跨任务 dataset memory 与 M4 corpus RAG 已分别落到 `memory/` 与
`memory/rag/`；实施摘要和复查记录见
[`src/计划/RAG记忆架构演示/03-implementation-notes/01-implementation-summary.md`](src/计划/RAG记忆架构演示/03-implementation-notes/01-implementation-summary.md)
与
[`src/计划/RAG记忆架构演示/03-implementation-notes/03-m4-post-review-2026-05-15.md`](src/计划/RAG记忆架构演示/03-implementation-notes/03-m4-post-review-2026-05-15.md)。
...
| 8 清理 / 拆分 / 中文化 | ✅ |
| Memory v2 / M4 Corpus RAG | ✅ 已落地；默认关闭，显式 `--memory-rag` / `DATA_AGENT_RAG=1` 开启 |

详见：

- [`src/计划/LangChain/MIGRATION_STATUS.md`](src/计划/LangChain/MIGRATION_STATUS.md) —— **single source of truth**，所有阶段细节、文件清单、测试 delta 都在此处。
- [`src/计划/LangChain/LANGCHAIN_MIGRATION_PLAN.md`](src/计划/LangChain/LANGCHAIN_MIGRATION_PLAN.md) —— v4 完整迁移方案。
- [`src/计划/RAG记忆架构演示/03-implementation-notes/01-implementation-summary.md`](src/计划/RAG记忆架构演示/03-implementation-notes/01-implementation-summary.md) —— Memory v2 与 M4 Corpus RAG 实施摘要。
- [`src/计划/LangChain/PHASE_05_GATEWAY_SMOKE_PLAN.md`](src/计划/LangChain/PHASE_05_GATEWAY_SMOKE_PLAN.md) —— gateway 能力探测方案。
- [`src/计划/LangChain/PHASE_6_TOOL_CALLING_DEFAULT_PLAN.md`](src/计划/LangChain/PHASE_6_TOOL_CALLING_DEFAULT_PLAN.md) —— 默认动作模式切换记录。
- [`src/计划/RAG记忆架构演示/03-implementation-notes/03-m4-post-review-2026-05-15.md`](src/计划/RAG记忆架构演示/03-implementation-notes/03-m4-post-review-2026-05-15.md) —— M4 Corpus RAG 执行后复查与本次修复记录。
...
7. **`step_index` 仅由 `model_node` 递增。** 其他节点只追加 `StepRecord`，
   不动计数器（D2 / §5.4）。
8. **`configs/react_baseline.example.yaml` 含 API 密钥。** 不要打开 / 提交 /
   把内容贴到任何地方（trace / 日志 / commit message）。本地运行使用
   `.gitignore` 保护的 `configs/local.yaml`，并只保留本地路径类覆盖项。
9. **Phase 状态以 `MIGRATION_STATUS.md` 为准。** 本 README 的 "Phase 状态"
