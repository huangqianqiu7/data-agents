...
| `AgentConfig` | `model` / `api_base` / `api_key` / `temperature` / `max_steps=60` / `max_replans=4` / `max_gate_retries=6` / `action_mode="tool_calling"` / `model_timeout_s=120` / `tool_timeout_s=180` / `max_model_retries=6` / `max_obs_chars=3000` / `max_context_tokens=24000` / `seed` 等 | 主循环、超时、上下文预算、决定性 |
