"""
Command-line interface for the DABench baseline project.

Provides four sub-commands:
  - ``status``          — show project layout and dataset summary
  - ``inspect-task``    — display a single task's metadata and context files
  - ``run-task``        — run the agent on one task
  - ``run-benchmark``   — run the agent on the full (or limited) dataset
"""
...
from data_agent_refactored.run.runner import (
    TaskRunArtifacts,
    create_run_output_dir,
    run_benchmark,
    run_single_task,
)
from data_agent_refactored.tools.filesystem import list_context_tree

PROJECT_ROOT: Path = Path(__file__).resolve().parents[2]
CONFIGS_DIR: Path = PROJECT_ROOT / "configs"
DATA_DIR: Path = PROJECT_ROOT / "data"
ARTIFACTS_DIR: Path = PROJECT_ROOT / "artifacts"
ARTIFACT_RUNS_DIR: Path = ARTIFACTS_DIR / "runs"
...
def run_task_command(
    ...
    artifacts = run_single_task(
        task_id=task_id, config=app_config, run_output_dir=run_output_dir
    )
...
def run_benchmark_command(
    ...
    with Progress(*progress_columns, console=console) as progress_bar:
