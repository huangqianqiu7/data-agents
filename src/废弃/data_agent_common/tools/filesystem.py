"""
Filesystem utilities for reading task context files.

All path operations enforce sandbox containment: relative paths are resolved
against the task's ``context_dir`` and must not escape it.
"""
...
def resolve_context_path(task: PublicTask, relative_path: str) -> Path:
    """Resolve *relative_path* inside the task context, rejecting path traversal."""
...
def list_context_tree(task: PublicTask, *, max_depth: int = 4) -> dict[str, Any]:
    """Return a recursive directory listing of the task context folder."""
...
def read_csv_preview(
    ...
    """Return the first *max_rows* rows of a CSV file as a dict."""
...
def read_json_preview(
    ...
    """Return a character-limited preview of a JSON file."""
    path = resolve_context_path(task, relative_path)
...
def read_doc_preview(
    ...
    """Return a character-limited preview of a text document."""
    path = resolve_context_path(task, relative_path)
