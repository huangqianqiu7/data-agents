"""
Sandboxed Python code execution via a subprocess.

User-provided code is run inside a separate process with captured
stdout / stderr and a hard timeout to prevent runaway execution.

The :data:`EXECUTE_PYTHON_TIMEOUT_SECONDS` constant is the single source of
truth for the public timeout; both the legacy ``ToolRegistry`` description
and the new ``descriptions.py`` byte-for-byte parity registry import it.
"""
...
# Safety limit: maximum seconds for execute_python code execution.
# Used by the legacy ToolRegistry description and the new
# data_agent_langchain.tools.descriptions parity registry — keep one source
# of truth (LANGCHAIN_MIGRATION_PLAN.md v4 E11).
...
def _capture_process_streams(
    ...
    """Redirect file-descriptor-level stdout/stderr to files, then restore."""
...
def _run_python_code(
    ...
    """Worker target executed in a child process."""
...
def execute_python_code(
    ...
    """Execute *code* in a subprocess rooted at *context_root*.

    Returns a dict with keys ``success``, ``output``, ``stderr``, and
