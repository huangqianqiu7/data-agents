"""
Public dataset accessor for the DABench benchmark.

Provides :class:`DABenchPublicDataset` which discovers, loads, and filters
tasks from a local directory tree following the ``task_NNNN/`` convention.
"""
...
from data_agent_common.benchmark.schema import PublicTask, TaskAssets, TaskRecord
from data_agent_common.exceptions import DatasetError, TaskNotFoundError
...
def _task_number(task_id: str) -> int:
    """Extract the numeric suffix from a task ID like ``task_42``."""
...
def _load_task_record(task_json_path: Path) -> TaskRecord:
    """Parse and validate a single ``task.json`` file."""
...
class DABenchPublicDataset:
    """Read-only accessor for the public DABench task dataset on disk."""
    root_dir: Path
